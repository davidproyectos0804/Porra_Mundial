...
const seedSemifinales = async () => {
  try {
    await mongoose.connect(process.env.MONGODB_URI);
    console.log('✅ Conectado a MongoDB Atlas');

    // Buscar la fase de Octavos
    const fase = await Fase.findOne({ nombre: 'Semifinales' });
    if (!fase) {
      console.error('❌ Fase Semifinales no encontrada');
      process.exit(1);
    }
    console.log('✅ Fase Semifinales encontrada');
    ...
    // =============================================
    const partidos = [
  { local: 'Inglaterra', visitante: 'Argentina', fecha: '2026-07-15T21:00:00' },  
];
    ...
    console.log('✅ 2 partidos de Semifinales insertados');
