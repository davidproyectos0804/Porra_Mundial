...
const seedOctavos = async () => {
  try {
    await mongoose.connect(process.env.MONGODB_URI);
    console.log('✅ Conectado a MongoDB Atlas');

    // Buscar la fase de Octavos
    const fase = await Fase.findOne({ nombre: 'Octavos' });
    if (!fase) {
      console.error('❌ Fase Octavos no encontrada');
      process.exit(1);
    }
    console.log('✅ Fase Octavos encontrada');
    ...
    // =============================================
    const partidos = [
  { local: 'Suiza', visitante: 'Colombia', fecha: '2026-07-07T22:00:00' },  
];
    ...
    console.log('✅ 16 partidos de Octavos insertados');
